...
def format_prometheus(data):
    ...
    for item in data:
        ...
        result += (
            f'uptimerobot_status{{c1_name="{item.get("friendly_name")}",c2_url="{item.get("url")}"'
        ...
        if item.get("status", 0) == 2 and item.get("response_times"):
            result += (
                f'uptimerobot_response_time{{name="{item.get("friendly_name")}",type="{item.get("type")}",'
...
def format_prometheus_accountdetails(data):
    result = (
        f'uptimerobot_accountdetails{{name="{data["email"]}",monitor_limit="{data["monitor_limit"]}",'
...
def format_prometheus_psp(data):
    ...
    for item in data:
        result += (
            f'uptimerobot_psp{{c1_name="{item.get("friendly_name")}",c2_custom_url="{item.get("custom_url")}",'
...
if __name__ == "__main__":
    if "UPTIMEROBOT_API_KEY" in os.environ:
        ...
        parser.add_argument(
            "apikey", help="Your uptimerobot.com API key. See account details."
        )
        parser.add_argument(
            "--server_name",
            "-s",
